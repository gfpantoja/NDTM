...
double separacion = 0.0; // Un valor negativo indica separaci�n, un valor positivo, solapamiento
...
int maxTime = 3600;
int nThreads = 8;
int modelInd = -2; // Se ejecutan todos lo modelos
string fileName = "";
...
public:
    ...
    int boxid, x1, x2, y1, y2;

    // Constructor

    Caja(const int& p_boxId, const int& p_x1, const int& p_y1, const int& p_x2, const int& p_y2) {
        boxid = p_boxId;
        x1 = p_x1;
        y1 = p_y1;
        x2 = p_x2;
...
public:
    ...
    CajaUPiezas(Caja* p_caja) {
        cantidad = 1;
        cajas = vector<Caja*>(1, p_caja);
        dx = (*p_caja).x2 - (*p_caja).x1;
        dy = (*p_caja).y2 - (*p_caja).y1;
        piezas = vector<PiezaG*>(0);
    }

    // M�todos

    void AgregarCaja(Caja* p_caja) {
        ++cantidad;
        cajas.push_back(p_caja);
    }
};
...
void AuxiliarCM(vector<double>& C21, vector<double>& M1, vector<Vec3>::iterator const& l1_it, vector<Parte>::iterator const& pp2_it, vector<Parte>::iterator const& pp1_it, int const& linea, vector<PiezaG>::iterator const& p1_it, vector<PiezaG>::iterator const& p2_it) {
    double maxVal = distancia(*l1_it, (*pp2_it).vertices.front());
    for (vector<Vec2>::iterator v_it = (*pp2_it).vertices.begin() + 1; v_it < (*pp2_it).vertices.end(); ++v_it) {
        double otroVal = distancia(*l1_it, *v_it);
        if (maxVal < otroVal) {
            maxVal = otroVal;
        }
    }
    C21.push_back(maxVal);
    M1.push_back(maxVal + abs((*pp1_it).lineas[linea].y) * Ly);
}
void CalcularParametros() {
    ...
    maxLx += separacion * (double)(totalPiezas - 1);
    ...
    for (vector<TipoPieza>::iterator t1_it = tiposPieza.begin(); t1_it < tiposPieza.end(); ++t1_it) {
        for (vector<PiezaG>::iterator p1_it = (*t1_it).piezas.begin(); p1_it < (*t1_it).piezas.end(); ++p1_it) {
            for (vector<Parte>::iterator pp1_it = (*p1_it).partes.begin(); pp1_it < (*p1_it).partes.end(); ++pp1_it) {
                int linea = 0;
                (*pp1_it).M = (*pp1_it).M0;
                for (vector<vector<vector<vector<bool>>>>::iterator sep4_it = (*pp1_it).PuedeSeparar.begin(); sep4_it < (*pp1_it).PuedeSeparar.end(); ++sep4_it, ++linea) { // linea 1
                    int tipo2 = 0;
                    ...
                    for (vector<vector<vector<bool>>>::iterator sep3_it = (*sep4_it).begin(); sep3_it < (*sep4_it).end(); ++sep3_it, ++tipo2, ++t2_it) { // tipo 2
                        ...
                        for (vector<vector<bool>>::iterator sep2_it = (*sep3_it).begin(); sep2_it < (*sep3_it).end(); ++sep2_it, ++pieza2, ++p2_it) { // pieza 2
                            ...
                            for (vector<bool>::iterator sep1_it = (*sep2_it).begin(); sep1_it < (*sep2_it).end(); ++sep1_it, ++parte2) { // parte 2
                                if (*sep1_it) {
                                    (*pp1_it).M[linea][tipo2][pieza2][parte2] += abs((*pp1_it).lineas[linea].x) * miLx;
                                    //(*pp1_it).M[linea][tipo2][pieza2][parte2] += abs((*pp1_it).lineas[linea].x) * (miLx - min((*p1_it).dmin.x + (*p2_it).dmax.x, (*p1_it).dmax.x + (*p2_it).dmin.x));
                                    //(*pp1_it).M[linea][tipo2][pieza2][parte2] += abs((*pp1_it).lineas[linea].x) * (miLx + min((*p1_it).dmin.x + (*p2_it).dmax.x, (*p1_it).dmax.x + (*p2_it).dmin.x));
                                }
                            }
                        }
...
void NDTM1(Solucion& sol) {
    ...
    try {
                                        ...
                                        for (vector<Parte>::iterator pp2_it = (*p2_it).partes.begin(); pp2_it < (*p2_it).partes.end(); ++pp2_it, ++ll) {
                                            ...
                                            for (vector<Vec3>::iterator l_it = (*pp_it).lineas.begin(); l_it < (*pp_it).lineas.end(); ++l_it, ++m) {
                                                if ((*pp_it).PuedeSeparar[m][ii][jj][ll]) {
                                                    model.add((*l_it).x * (x[ii][jj][kk] - x[i][j][k])
                                                        + (*l_it).y * (y[ii][jj][kk] - y[i][j][k])
                                            ...
                                            for (vector<Vec3>::iterator l_it = (*pp2_it).lineas.begin(); l_it < (*pp2_it).lineas.end(); ++l_it, ++m) {
                                                if ((*pp2_it).PuedeSeparar[m][i][j][l]) {
                                                    model.add((*l_it).x * (x[i][j][k] - x[ii][jj][kk])
                                                        + (*l_it).y * (y[i][j][k] - y[ii][jj][kk])
        ...
        /*
        IloNumVarArray3 aux(env, tiposPieza.size());
        i = 0;
        for (vector<TipoPieza>::iterator t_it = tiposPieza.begin(); t_it < tiposPieza.end(); ++t_it, ++i) {
            int j = 0;
            aux[i] = IloNumVarArray2(env, (*t_it).piezas.size());
            for (vector<PiezaG>::iterator p_it = (*t_it).piezas.begin(); p_it < (*t_it).piezas.end(); ++p_it, ++j) {
                aux[i][j] = IloNumVarArray(env, (*t_it).cantidad0, 0, maxLx, ILOFLOAT);
            }
        }
        i = 0;
        for (vector<TipoPieza>::iterator t_it = tiposPieza.begin(); t_it < tiposPieza.end(); ++t_it, ++i) {
            for (int k = 1; k < (*t_it).cantidad0; ++k) {
                for (int j1 = 0; j1 < (*t_it).piezas.size(); ++j1) {
                    for (int j2 = 0; j2 < (*t_it).piezas.size(); ++j2) {
                        model.add(aux[i][j1][k] <= maxLx * w[i][j1][k]);
                        model.add(aux[i][j1][k] <= maxLx * w[i][j1][k - 1]);
                        model.add(aux[i][j1][k] >= x[i][j1][k] - (2 - w[i][j1][k] - w[i][j1][k - 1]) * maxLx);
                        model.add(aux[i][j1][k] <= x[i][j1][k]);
                        model.add(aux[i][j1][k - 1] <= maxLx * w[i][j1][k]);
                        model.add(aux[i][j1][k - 1] <= maxLx * w[i][j1][k - 1]);
                        model.add(aux[i][j1][k - 1] >= x[i][j1][k - 1] - (2 - w[i][j1][k] - w[i][j1][k - 1]) * maxLx);
                        model.add(aux[i][j1][k - 1] <= x[i][j1][k - 1]);
                        model.add(aux[i][j1][k] >= aux[i][j2][k - 1]); // Simetr�a
                    }
                }
            }
        }
        */
        ...
        //cplex.setParam(IloCplex::EpGap, 1e-6);
        cplex.setOut(fout);
        cplex.setWarning(fout);
        cplex.setError(fout);
        //cplex.setOut(env.getNullStream());
        //cplex.exportModel("ModeloLinealMB1.lp");
...
void NDTM2(Solucion& sol) {
    ...
    try {
        ...
        for (vector<TipoPieza>::iterator t_it = tiposPieza.begin(); t_it < tiposPieza.end(); ++t_it) {
            ...
            y[i] = IloNumVarArray2(env, (*t_it).piezas.size());
            w[i] = IloBoolVarArray2(env, (*t_it).piezas.size());
            for (vector<PiezaG>::iterator p_it = (*t_it).piezas.begin(); p_it < (*t_it).piezas.end(); ++p_it) {
                x[i][j] = IloNumVarArray(env, (*t_it).cantidad0, (*p_it).dmin.x, maxLx - (*p_it).dmax.x, ILOFLOAT);
                y[i][j] = IloNumVarArray(env, (*t_it).cantidad0, (*p_it).dmin.y, Ly - (*p_it).dmax.y, ILOFLOAT);
                w[i][j] = IloBoolVarArray(env, (*t_it).cantidad0);
                ++j;
        ...
        for (vector<TipoPieza>::iterator t_it = tiposPieza.begin(); t_it < tiposPieza.end(); ++t_it) {
            ...
            for (vector<PiezaG>::iterator p_it = (*t_it).piezas.begin(); p_it < (*t_it).piezas.end(); ++p_it) {
                ...
                for (int k = 0; k < (*t_it).cantidad0; ++k) {
                    u[i][j][k] = IloBoolVarArray6(env, (*p_it).partes.size());
                    int l = 0;
                    for (vector<Parte>::iterator pp_it = (*p_it).partes.begin(); pp_it < (*p_it).partes.end(); ++pp_it) {
                        u[i][j][k][l] = IloBoolVarArray5(env, (*pp_it).lineas.size());
                        int m = 0;
                        for (vector<Vec3>::iterator l_it = (*pp_it).lineas.begin(); l_it < (*pp_it).lineas.end(); ++l_it) {
                            u[i][j][k][l][m] = IloBoolVarArray4(env, tiposPieza.size());
                            int ii = 0;
                            for (vector<TipoPieza>::iterator t2_it = tiposPieza.begin(); t2_it < tiposPieza.end(); ++t2_it) {
                                u[i][j][k][l][m][ii] = IloBoolVarArray3(env, (*t2_it).piezas.size());
                                        ...
                                        for (vector<Parte>::iterator pp2_it = (*p2_it).partes.begin(); pp2_it < (*p2_it).partes.end(); ++pp2_it, ++ll) {
                                            ...
                                            for (vector<Vec3>::iterator l_it = (*pp_it).lineas.begin(); l_it < (*pp_it).lineas.end(); ++l_it, ++m) {
                                                if ((*pp_it).PuedeSeparar[m][ii][jj][ll]) {
                                                    model.add((*l_it).x * (x[ii][jj][kk] - x[i][j][k])
                                                        + (*l_it).y * (y[ii][jj][kk] - y[i][j][k])
                                                        + (*pp_it).C2[m][ii][jj][ll] <= (*pp_it).M[m][ii][jj][ll] * (1 - u[i][j][k][l][m][ii][jj][kk][ll]) - separacion);
                                                    sumu += u[i][j][k][l][m][ii][jj][kk][ll];
                                                }
                                            }
                                            m = 0;
                                            for (vector<Vec3>::iterator l_it = (*pp2_it).lineas.begin(); l_it < (*pp2_it).lineas.end(); ++l_it, ++m) {
                                                if ((*pp2_it).PuedeSeparar[m][i][j][l]) {
                                                    model.add((*l_it).x * (x[i][j][k] - x[ii][jj][kk])
                                                        + (*l_it).y * (y[i][j][k] - y[ii][jj][kk])
                                                        + (*pp2_it).C2[m][i][j][l] <= (*pp2_it).M[m][i][j][l] * (1 - u[ii][jj][kk][ll][m][i][j][k][l]) - separacion);
                                                    sumu += u[ii][jj][kk][ll][m][i][j][k][l];
                                                }
                                            }
                                            int miM = (*pp_it).lineas.size() + (*pp2_it).lineas.size();
                                            model.add(sumu <= miM * w[i][j][k]);
                                            model.add(sumu <= miM * w[ii][jj][kk]);
                                            model.add(sumu >= w[i][j][k] + w[ii][jj][kk] - 1);
                                            sumu.end();
                                        }
                                    }
...
void NDTM_Mod1(Solucion& sol) {
    ...
    try {
        ...
        IloBoolVarArray6 vx(env, tiposPieza.size());
        IloBoolVarArray6 vy(env, tiposPieza.size());
        i = 0;
        for (vector<TipoPieza>::iterator t_it = tiposPieza.begin(); t_it < tiposPieza.end(); ++t_it, ++i) {
            int j = 0;
            vx[i] = IloBoolVarArray5(env, (*t_it).piezas.size());
            vy[i] = IloBoolVarArray5(env, (*t_it).piezas.size());
            for (vector<PiezaG>::iterator p_it = (*t_it).piezas.begin(); p_it < (*t_it).piezas.end(); ++p_it, ++j) {
                vx[i][j] = IloBoolVarArray4(env, (*t_it).cantidad0);
                vy[i][j] = IloBoolVarArray4(env, (*t_it).cantidad0);
                for (int k = 0; k < (*t_it).cantidad0; ++k) {
                    int ii = 0;
                    vx[i][j][k] = IloBoolVarArray3(env, tiposPieza.size());
                    vy[i][j][k] = IloBoolVarArray3(env, tiposPieza.size());
                    for (vector<TipoPieza>::iterator t2_it = tiposPieza.begin(); t2_it < tiposPieza.end(); ++t2_it, ++ii) {
                        int jj = 0;
                        vx[i][j][k][ii] = IloBoolVarArray2(env, (*t2_it).piezas.size());
                        vy[i][j][k][ii] = IloBoolVarArray2(env, (*t2_it).piezas.size());
                        for (vector<PiezaG>::iterator p2_it = (*t2_it).piezas.begin(); p2_it < (*t2_it).piezas.end(); ++p2_it, ++jj) {
                            vx[i][j][k][ii][jj] = IloBoolVarArray(env, (*t2_it).cantidad0);
                            vy[i][j][k][ii][jj] = IloBoolVarArray(env, (*t2_it).cantidad0);
                        }
                    }
                }
            }
        ...
        // Restricciones: Separaci�n por BB parte 1
                ...
                for (int k = 0; k < (*t_it).cantidad0; ++k) {
                    ...
                    for (vector<TipoPieza>::iterator t2_it = tiposPieza.begin(); t2_it < tiposPieza.end(); ++t2_it, ++ii) {
                        ...
                        for (vector<PiezaG>::iterator p2_it = (*t2_it).piezas.begin(); p2_it < (*t2_it).piezas.end(); ++p2_it, ++jj) {
                            for (int kk = 0; kk < (*t2_it).cantidad0; ++kk) {
                                if (i != ii || j != jj || k != kk) {
                                    model.add(maxLx * (1 - vx[i][j][k][ii][jj][kk]) + x[i][j][k] - (*p_it).dmin.x >= x[ii][jj][kk] + (*p2_it).dmax.x);
                                    model.add(Ly * (1 - vy[i][j][k][ii][jj][kk]) + y[i][j][k] - (*p_it).dmin.y >= y[ii][jj][kk] + (*p2_it).dmax.y);
                                }
                            }
                        }
                    }
        ...
        // Restricciones: No solapamiento y separaci�n por BB
                                ...
                                if (ii > i || (ii == i && jj > j && (*t_it).cantidad0 > 1) || (ii == i && jj == j && kk > k)) {
                                    ...
                                    for (vector<Parte>::iterator pp_it = (*p_it).partes.begin(); pp_it < (*p_it).partes.end(); ++pp_it, ++l) {
                                        ...
                                        for (vector<Parte>::iterator pp2_it = (*p2_it).partes.begin(); pp2_it < (*p2_it).partes.end(); ++pp2_it, ++ll) {
                                            ...
                                            int miM = (*pp_it).lineas.size() + (*pp2_it).lineas.size() + 4;
                                            model.add(sumu + vx[i][j][k][ii][jj][kk] + vx[ii][jj][kk][i][j][k] + vy[i][j][k][ii][jj][kk] + vy[ii][jj][kk][i][j][k] <= miM * w[i][j][k]);
                                            model.add(sumu + vx[i][j][k][ii][jj][kk] + vx[ii][jj][kk][i][j][k] + vy[i][j][k][ii][jj][kk] + vy[ii][jj][kk][i][j][k] <= miM * w[ii][jj][kk]);
                                            model.add(sumu + vx[i][j][k][ii][jj][kk] + vx[ii][jj][kk][i][j][k] + vy[i][j][k][ii][jj][kk] + vy[ii][jj][kk][i][j][k] >= w[i][j][k] + w[ii][jj][kk] - 1);
                                            sumu.end();
                                        }
                                    }
                                }
...
void NDTM_Mod2(Solucion& sol) {
    ...
    try {
        ...
        IloBoolVarArray6 vx(env, tiposPieza.size());
        IloBoolVarArray6 vy(env, tiposPieza.size());
        IloBoolVarArray6 s(env, tiposPieza.size());
        i = 0;
        for (vector<TipoPieza>::iterator t_it = tiposPieza.begin(); t_it < tiposPieza.end(); ++t_it, ++i) {
            int j = 0;
            vx[i] = IloBoolVarArray5(env, (*t_it).piezas.size());
            vy[i] = IloBoolVarArray5(env, (*t_it).piezas.size());
            s[i] = IloBoolVarArray5(env, (*t_it).piezas.size());
            for (vector<PiezaG>::iterator p_it = (*t_it).piezas.begin(); p_it < (*t_it).piezas.end(); ++p_it, ++j) {
                vx[i][j] = IloBoolVarArray4(env, (*t_it).cantidad0);
                vy[i][j] = IloBoolVarArray4(env, (*t_it).cantidad0);
                s[i][j] = IloBoolVarArray4(env, (*t_it).cantidad0);
                for (int k = 0; k < (*t_it).cantidad0; ++k) {
                    int ii = 0;
                    vx[i][j][k] = IloBoolVarArray3(env, tiposPieza.size());
                    vy[i][j][k] = IloBoolVarArray3(env, tiposPieza.size());
                    s[i][j][k] = IloBoolVarArray3(env, tiposPieza.size());
                    for (vector<TipoPieza>::iterator t2_it = tiposPieza.begin(); t2_it < tiposPieza.end(); ++t2_it, ++ii) {
                        int jj = 0;
                        vx[i][j][k][ii] = IloBoolVarArray2(env, (*t2_it).piezas.size());
                        vy[i][j][k][ii] = IloBoolVarArray2(env, (*t2_it).piezas.size());
                        s[i][j][k][ii] = IloBoolVarArray2(env, (*t2_it).piezas.size());
                        for (vector<PiezaG>::iterator p2_it = (*t2_it).piezas.begin(); p2_it < (*t2_it).piezas.end(); ++p2_it, ++jj) {
                            vx[i][j][k][ii][jj] = IloBoolVarArray(env, (*t2_it).cantidad0);
                            vy[i][j][k][ii][jj] = IloBoolVarArray(env, (*t2_it).cantidad0);
                            s[i][j][k][ii][jj] = IloBoolVarArray(env, (*t2_it).cantidad0);
                        }
                    }
                }
            }
                                ...
                                if (ii > i || (ii == i && jj > j && (*t_it).cantidad0 > 1) || (ii == i && jj == j && kk > k)) {
                                    ...
                                    for (vector<Parte>::iterator pp_it = (*p_it).partes.begin(); pp_it < (*p_it).partes.end(); ++pp_it, ++l) {
                                        ...
                                        for (vector<Parte>::iterator pp2_it = (*p2_it).partes.begin(); pp2_it < (*p2_it).partes.end(); ++pp2_it, ++ll) {
                                            ...
                                            int miM2 = (*pp_it).lineas.size() + (*pp2_it).lineas.size();
                                            int miM = miM2 + 4;
                                            model.add(sumu + vx[i][j][k][ii][jj][kk] + vx[ii][jj][kk][i][j][k] + vy[i][j][k][ii][jj][kk] + vy[ii][jj][kk][i][j][k] <= miM * w[i][j][k]);
                                            model.add(sumu + vx[i][j][k][ii][jj][kk] + vx[ii][jj][kk][i][j][k] + vy[i][j][k][ii][jj][kk] + vy[ii][jj][kk][i][j][k] <= miM * w[ii][jj][kk]);
                                            model.add(sumu + vx[i][j][k][ii][jj][kk] + vx[ii][jj][kk][i][j][k] + vy[i][j][k][ii][jj][kk] + vy[ii][jj][kk][i][j][k] >= w[i][j][k] + w[ii][jj][kk] - 1);
                                            model.add(s[i][j][k][ii][jj][kk] <= vx[i][j][k][ii][jj][kk] + vx[ii][jj][kk][i][j][k] + vy[i][j][k][ii][jj][kk] + vy[ii][jj][kk][i][j][k]);
                                            model.add(4 * s[i][j][k][ii][jj][kk] >= vx[i][j][k][ii][jj][kk] + vx[ii][jj][kk][i][j][k] + vy[i][j][k][ii][jj][kk] + vy[ii][jj][kk][i][j][k]);
                                            model.add(sumu <= miM2 * (1 - s[i][j][k][ii][jj][kk]));
                                            model.add(sumu >= s[i][j][k][ii][jj][kk] - 1);
                                            sumu.end();
                                        }
                                    }
                                }
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc - 1; i += 2) {
        if (argc - 1 >= i + 1) {
            ...
            else if (string(argv[i]) == "-m") modelInd = atoi(argv[i + 1]);
            else if (string(argv[i]) == "-area") areaTotalPiezas = atof(argv[i + 1]);
            else if (string(argv[i]) == "-sep") separacion = atof(argv[i + 1]);
            else {
    ...
    if (modelInd == -1) {
        cout << "Entra a NDTM1" << endl;
        timeIni = std::chrono::system_clock::now();
        Solucion S_NDTM1;
        NDTM1(S_NDTM1);
        cout << "Sale de NDTM1" << endl;
    }
    else if (modelInd == 0) {
        cout << "Entra a NDTM" << endl;
        timeIni = std::chrono::system_clock::now();
        Solucion S_NDTM;
        NDTM2(S_NDTM);
        cout << "Sale de NDTM" << endl;
    }
    else if (modelInd == 1) {
        cout << "Entra a Mod1" << endl;
        timeIni = std::chrono::system_clock::now();
        Solucion S_mod1;
        NDTM_Mod1(S_mod1);
        cout << "Sale de Mod1" << endl;
    }
    else if (modelInd == 2) {
        cout << "Entra a Mod2" << endl;
        timeIni = std::chrono::system_clock::now();
        Solucion S_mod2;
        NDTM_Mod2(S_mod2);
        cout << "Sale de Mod2" << endl;
    }
}
//END
